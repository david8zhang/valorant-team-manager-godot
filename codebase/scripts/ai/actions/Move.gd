...
func get_tiles_within_radius(center_pos: Vector2, radius: int, game_round: GameRound) -> Array[Vector2]:
	var map = game_round.map as Map
	var tiles_to_consider: Array[Vector2] = []
	...
	for x in range(min_x, max_x + 1):
		for y in range(min_y, max_y + 1):
			...
			if !game_round.pathfinding.can_find_path(center_pos, tile_pos):
				continue
			var dist_squared = (tile_pos - center_pos).length_squared()
